...
# --- LlamaNote Modules ---
from loggerConf import get_logger_conf, log_execution_time, log_resource_usage, ConsoleOutput
...
class LocalTransformerBackend(LLMBackend):
    ...
    def __init__(self,
        ...
        # Use layer_split_config if provided, else derive from memory_config
    ...
    def _load_quantized_model(self, trust_remote_code: bool) -> bool:
        ...
        if bnb_config is None:
            self.logger.warning(f"Quantization method '{self.quant_config.method}' selected but bnb_config is None. Loading standard model.")
    ...
    def _load_standard_model(self, trust_remote_code: bool) -> bool:
        ...
        try:
            ...
            self.logger.info("Successfully loaded model in standard mode")
            return True
    ...
    def _load_cpu_model(self, trust_remote_code: bool) -> bool:
        ...
        try:
            model = AutoModelForCausalLM.from_pretrained(
                ...
                device_map="cpu",
                cache_dir=str(CACHE_DIR),
                low_cpu_mem_usage=False, # No need for low_cpu_mem_usage if map is CPU
                trust_remote_code=trust_remote_code
            )
            ...
            self.model = model
            self.tokenizer = tokenizer
            self.device_map = {"model": "cpu"} # Explicitly set
    ...
    def _post_load_setup(self):
        ...
        if self.tokenizer and self.tokenizer.pad_token is None:
            # Check if eos_token exists and is not None before assigning
            if self.tokenizer.eos_token is not None:
                self.tokenizer.pad_token = self.tokenizer.eos_token
                self.logger.debug("Set pad_token to eos_token")
            else:
                # Handle models that might not have eos_token either (e.g., some T5 variants)
                # Setting to 0, but this might need model-specific handling
                self.tokenizer.pad_token_id = 0 
                self.logger.warning("Model tokenizer missing pad_token and eos_token. Setting pad_token_id to 0.")
        ...
        # Update token calculator with model config's max context
        self.token_calculator.model_max_context = self.model_config.max_context
        
        # Update response filter with model config
    ...
    def _create_device_map(self) -> Dict[str, Any]:
        ...
        if self.split_config.gpu_layers > 0:
             self.logger.warning("Manual 'gpu_layers' for Transformers backend is complex; "
                               "using 'auto' with 'max_gpu_memory' constraint instead.")
    ...
    def _log_model_info(self):
        ...
        try:
            ...
            if self.quant_config.method == "4bit":
                param_size_gb = (param_count / 2) / (1024 ** 3) # ~0.5 bytes/param
            elif self.quant_config.method == "8bit":
                 param_size_gb = (param_count) / (1024 ** 3) # ~1 byte/param
            elif self.quant_config.method == "16bit" or self.quant_config.compute_dtype in [torch.float16, torch.bfloat16]:
                 param_size_gb = (param_count * 2) / (1024 ** 3) # 2 bytes/param
            else: # float32 or unknown
                 param_size_gb = (param_count * 4) / (1024 ** 3) # 4 bytes/param
            
            self.logger.info(f"Model loaded: {param_count:,} parameters (~{param_size_gb:.2f} GB)")
        except Exception as e:
             self.logger.warning(f"Could not calculate model parameter count: {e}")
        ...
        if self.device_map:
            ...
            self.logger.info(f"  (Total modules mapped: {total_modules})")
    ...
    def generate(self,
        ...
        if hp.max_new_tokens is None:
            hp.max_new_tokens = self.token_calculator.calculate_max_new_tokens(
                input_text=prompt, 
                model_max_context=self.model_config.max_context
            )
        else:
            # Ensure max_new_tokens doesn't exceed model context
            hp.max_new_tokens = min(hp.max_new_tokens,
                                     self.token_calculator.calculate_max_new_tokens(
                                         input_text=prompt, 
                                         model_max_context=self.model_config.max_context
                                     ))

            
        self.logger.debug(f"Generating with max_new_tokens={hp.max_new_tokens}, "
                    f"temperature={hp.temperature}, top_p={hp.top_p}")
        
        # Tokenize input
        inputs = self.tokenizer(prompt, return_tensors="pt", truncation=True, 
                                max_length=self.model_config.max_context - hp.max_new_tokens - 10) # Add buffer
        ...
        # Find the prompt in the output (it might not be an exact prefix if special tokens were added)
        prompt_in_output_idx = full_output.find(prompt)
        if prompt_in_output_idx != -1:
             raw_output = full_output[prompt_in_output_idx + len(prompt):].strip()
        else:
        ...
        if torch.cuda.is_available():
            try:
                # Peak memory might be more useful
    ...
    def _get_model_device(self) -> torch.device:
        ...
        if self.model is None: return torch.device("cpu")
        
        # For models loaded with device_map='auto' or Accelerator
        if hasattr(self.model, 'device'):
            return self.model.device
            
        # For models loaded with explicit device_map
        if hasattr(self.model, 'hf_device_map'):
            try:
                # Find the first layer's device
    ...
    def process_with_chat_template(self,
        ...
        try:
            if hasattr(self.tokenizer, 'apply_chat_template') and self.tokenizer.chat_template:
                ...
                # Fallback formatting
                self.logger.warning(f"Model {self.model_id} has no chat template. Using basic formatting.")
                prompt = f"SYSTEM: {system_prompt}\n\nUSER: {user_message}\n\nASSISTANT:"
        
        except Exception as e:
            self.logger.warning(f"Chat template application failed: {e}, using fallback formatting.")
            prompt = f"SYSTEM: {system_prompt}\n\nUSER: {user_message}\n\nASSISTANT:"
    ...
    def __del__(self):
        ...
        try:
            ...
            print(f"Error during LocalTransformerBackend __del__: {e}", file=sys.stderr)
...
class OpenAIBackend(LLMBackend):
    ...
    def load_model(self, **kwargs) -> bool:
        ...
        try:
            ...
            self.client.models.list() # Test API key
            self.logger.info("OpenAI API key verified.")
            return True
        except Exception as e:
            self.logger.error(f"OpenAI API key is invalid or connection failed: {e}", exc_info=True)
            ConsoleOutput.error(f"OpenAI API key check failed. Check key and network access.")
            return False
    ...
    def _map_hyperparams(self, hp: HyperparameterConfig) -> Dict[str, Any]:
        ...
        if hp.temperature is not None:
             # Ensure temperature is within valid range (e.g., 0 to 2)
             params["temperature"] = max(0.0, min(2.0, hp.temperature))
        if hp.top_p is not None:
             # OpenAI API often expects top_p < 1.0 or None
             params["top_p"] = max(0.01, min(0.999, hp.top_p)) if hp.top_p < 1.0 else None
        ...
        if hp.repetition_penalty is not None and hp.repetition_penalty != 1.0:
             ...
             params["presence_penalty"] = max(-2.0, min(2.0, hp.repetition_penalty - 1.0))
             self.logger.debug(f"Mapping repetition_penalty {hp.repetition_penalty} to presence_penalty {params['presence_penalty']}")

        # Note: OpenAI API doesn't support top_k, do_sample (implied by temp>0), 
        # length_penalty, no_repeat_ngram_size, etc. in the same way.
    ...
    def process_with_chat_template(self, system_prompt: str, user_message: str, hyperparams: Optional[HyperparameterConfig] = None, **kwargs) -> GenerationResult:
        ...
        try:
            ...
            self.logger.error(f"OpenAI API chat call failed: {e}", exc_info=True)
            raise

    def unload_model(self):
        # No unloading needed, but clear client
        del self.client
        self.client = None
        pass
...
class GoogleAIBackend(LLMBackend):
    ...
    def load_model(self, **kwargs) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Initialized GoogleAIBackend for model: {self.model_specifier}")
            return True
        except Exception as e:
            ...
            if "API_KEY_INVALID" in str(e) or "API key not valid" in str(e):
                 ...
                 ConsoleOutput.error(f"Google API key check failed: {e}")
            return False
    ...
    def _map_hyperparams(self, hp: HyperparameterConfig) -> Dict[str, Any]:
        ...
        if not hp.do_sample:
            ...
            gen_config_params["temperature"] = hp.temperature

        if hp.max_new_tokens is not None:
            gen_config_params["max_output_tokens"] = hp.max_new_tokens
        
        if hp.top_p is not None:
             # Google API seems to accept top_p=1.0
             gen_config_params["top_p"] = hp.top_p
    ...
    def generate(self, prompt: str, hyperparams: Optional[HyperparameterConfig] = None, **kwargs) -> GenerationResult:
        ...
        hp = hyperparams or self.hyperparams
        generation_config = genai.types.GenerationConfig(**self._map_hyperparams(hp))
        
        # Use the model instance initialized during load_model
        # If a system prompt was set via process_with_chat_template, 
        # self.model might already be configured with it.
        # For plain .generate(), we assume no system prompt unless previously set.
        model_to_use = self.model
        if self._system_prompt:
             # Re-init model if system prompt exists but isn't the one this model has
             # This is slightly clunky, Google's API prefers chat sessions for this
             if self.model.system_instruction is None: # Assumes default model has None
                  model_to_use = genai.GenerativeModel(
                       self.model_specifier, 
                       system_instruction=self._system_prompt
                  )
                  self.logger.debug("Re-creating model instance for generate() with system_prompt.")
        ...
        try:
            ...
            if not response.parts:
                 raw_output = "[Blocked by Google Safety Settings]"
            ...
            self.logger.error(f"Google AI API call failed: {e}", exc_info=True)
            raise
    ...
    def process_with_chat_template(self, system_prompt: str, user_message: str, hyperparams: Optional[HyperparameterConfig] = None, **kwargs) -> GenerationResult:
        ...
        if self._system_prompt != system_prompt:
            self.logger.info(f"Setting new system prompt for Google AI model: {system_prompt[:50]}...")
            ...
            try:
                 ...
                 self.logger.error(f"Failed to set system prompt for Google AI: {e}. Will prepend to user message.")
                 self.model = genai.GenerativeModel(self.model_specifier) # Ensure base model
                 user_message = f"System Instruction: {system_prompt}\n\nUser Request: {user_message}" # Fallback
        
        # Use the generate method, which will now use the configured self.model
...
class AnthropicBackend(LLMBackend):
    ...
    def load_model(self, **kwargs) -> bool:
        ...
        try:
            self.client = anthropic.Anthropic(api_key=self.api_key, http_client=httpx.Client(proxies=None)) # Explicitly no proxies if needed
            # Test API key by counting tokens
            self.client.count_tokens("test") 
            self.logger.info("Anthropic API key verified.")
            return True
        except Exception as e:
            self.logger.error(f"Anthropic API key is invalid or connection failed: {e}", exc_info=True)
            ConsoleOutput.error(f"Anthropic API key check failed: {e}")
            return False
    ...
    def _map_hyperparams(self, hp: HyperparameterConfig) -> Dict[str, Any]:
        ...
        params["max_tokens"] = hp.max_new_tokens or 2048 # Default if None
        
        if hp.temperature is not None:
             params["temperature"] = max(0.0, min(1.0, hp.temperature)) # Anthropic range 0.0-1.0
        if hp.top_p is not None:
             params["top_p"] = max(0.01, min(1.0, hp.top_p)) # Anthropic range 0.0-1.0
        if hp.top_k is not None and hp.top_k > 0:
             params["top_k"] = hp.top_k
             
        # Note: Anthropic doesn't support repetition_penalty
        if hp.repetition_penalty != 1.0:
    ...
    def generate(self, prompt: str, hyperparams: Optional[HyperparameterConfig] = None, **kwargs) -> GenerationResult:
        ...
        start_time = time.time()
        try:
            # Use process_with_chat_template logic as Anthropic API prefers messages format
            # and 'system' param for system prompt
            return self.process_with_chat_template(
                system_prompt="You are a helpful assistant.", # Default system prompt for raw generate
                user_message=prompt,
                hyperparams=hyperparams,
                **kwargs
            )
            ...
            self.logger.error(f"Anthropic API call failed: {e}", exc_info=True)
            raise
    ...
    def process_with_chat_template(self, system_prompt: str, user_message: str, hyperparams: Optional[HyperparameterConfig] = None, **kwargs) -> GenerationResult:
        ...
        try:
            ...
            self.logger.error(f"Anthropic API chat call failed: {e}", exc_info=True)
            raise
    ...
    try:
        if provider_lower == "local":
            ...
            if model_entry:
                ...
                logger.warning(f"Model {model_specifier} not in registry. Using default config.")
                model_conf = ModelConfig(name=model_specifier.split('/')[-1], model_id=model_specifier)
            ...
            return LocalTransformerBackend(
             ...
             # GGUF config must be passed via kwargs, or constructed here
             # We assume model_specifier is the path
             gguf_config = LlamaCppConfig(
                 model_path=Path(model_specifier),
                 n_ctx=hyperparameters.max_length or 2048, 
                 n_gpu_layers=layer_split_config.gpu_layers if layer_split_config else -1
             )
        ...
        * # ... implementation ...
        ...
        logger.error(f"Failed to create backend for {provider}: {e}", exc_info=True)
    ...
    def process_batch(self,
        ...
        with LoggingProgress(self.logger, "Processing batch", len(texts)) as progress:
            for i, text in enumerate(texts):
                ...
                # Clear cache between items (if local GPU)
                if self.llm_backend.provider_identifier == "local" and torch.cuda.is_available():
                    torch.cuda.empty_cache()
                
                progress.update(1, f"Item {i+1}/{len(texts)}")
                ConsoleOutput.progress_bar(i+1, len(texts), prefix="Processing")
