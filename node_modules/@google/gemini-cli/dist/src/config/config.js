...
export async function loadCliConfig(settings, sessionId, argv, cwd = process.cwd()) {
    ...
    if (enableMessageBusIntegration) {
        ...
        debugLogger.debug(`Default decision: ${policyEngineConfig.defaultDecision}`);
        ...
        if (policyEngineConfig.rules && policyEngineConfig.rules.length > 0) {
            ...
            policyEngineConfig.rules.forEach((rule, index) => {
                debugLogger.debug(`  [${index}] toolName: ${rule.toolName || '*'}, decision: ${rule.decision}, priority: ${rule.priority}, argsPattern: ${rule.argsPattern ? rule.argsPattern.source : 'none'}`);
            });
        }
        debugLogger.debug('===================================');
    }
    ...
    const excludeTools = mergeExcludeTools(settings, extensionManager.getExtensions(), extraExcludes.length > 0 ? extraExcludes : undefined);
